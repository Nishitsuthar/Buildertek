({
        ...
        action.setCallback(this, function(response){
            ...
            if(state === "SUCCESS"){
                ...
                // alert('** result.sObjectRecordsList  ---------->:');
                 console.log('** result.sObjectRecordsList --------------->:' +
                ...
                component.set("v.sObjectRecords",JSON.parse(result.sObjectRecordsList));
                
                helper.formatDataByGroups(component,event,helper,groupByData,result.fieldtypemap,JSON.parse(result.sObjectRecordsList));
    ...
    formatDataByGroups : function(component,event,helper,mapData,fieldtypemap,sObjectRecordsList){
        ...
        for (var i in mapData) {
            ...
            if(component.get("v.groupByPhasetoggle")){
                ...
                */
                    if (!recordsMap.has(mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c + '(#&%*)' + mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c)) {
                        recordsMap.set(mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c + '(#&%*)' + mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c, []);
                    }
                    mapData[i]['sheetrecord']['showIcon'] = mapData[i]['isShowIcon']
                    recordsMap.get(mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c + '(#&%*)' + mapData[i].sheetrecord.buildertek__BL_SERVICE_CATEGORY__c).push(JSON.parse(JSON.stringify(mapData[i].sheetrecord)));
                }else{
                    if (!recordsMap.has('No Service Category')) {
                        recordsMap.set('No Service Category', []);
                    }
                    mapData[i]['sheetrecord']['showIcon'] = mapData[i]['isShowIcon']
                    recordsMap.get('No Service Category').push(JSON.parse(JSON.stringify(mapData[i].sheetrecord)));
                }
       ...
       // $A.get('e.force:refreshView').fire();
        if (!component.get('v.massUpdateEnable') && component.get('v.showFabricationDetails') == true)
        {
            helper.calculateCostAdjustment(component, event, helper);
        }
         // $A.get('e.force:refreshView').fire();
    ...
    checkFabricationTaxes: function (component, event, helper) 
    {
        ...
        action.setCallback(this, function (response) 
        {
            ...
            if (state === "SUCCESS") 
            {
                ...
                if(fabricationCheck != null && fabricationCheck  != undefined && fabricationCheck == true)
                {
    ...
    calculateCostAdjustment: function (component, event, helper) 
    {
       ...
       // $A.get('e.force:refreshView').fire();
            ...
            bomLineFields != null && bomLineFields != undefined && bomLineFields.length > 0) 
        {
            for (var i = 0; i < adjustmentTableData.length; i++) 
            {
                ...
                if (adjustmentTableData[i].buildertek__Fabrication_Install_Rate__c != null && adjustmentTableData[i].buildertek__Fabrication_Install_Rate__c != undefined) {
                    fabricationAndIstallRate = JSON.parse(adjustmentTableData[i].buildertek__Fabrication_Install_Rate__c) ;
                }

                if (adjustmentTableData[i].buildertek__Install_Only__c != null && adjustmentTableData[i].buildertek__Install_Only__c != undefined) {
                    installOnly = JSON.parse(adjustmentTableData[i].buildertek__Install_Only__c) ;
                }
                ...
                if (adjustmentTableData[i].buildertek__Mark_Up_Non_Slab_or_Labor__c != null && adjustmentTableData[i].buildertek__Mark_Up_Non_Slab_or_Labor__c != undefined) {
                    nonSlabMarkup = JSON.parse(adjustmentTableData[i].buildertek__Mark_Up_Non_Slab_or_Labor__c) ;
                }

                if (adjustmentTableData[i].buildertek__Tax_on_Fabrication_Required__c != null && adjustmentTableData[i].buildertek__Tax_on_Fabrication_Required__c != undefined) {
                    taxOnFabRequired = adjustmentTableData[i].buildertek__Tax_on_Fabrication_Required__c ;
                }
            }
        }
        ...
        {
            var discountCol = '{"label": "Discount", "value": "buildertek__Discount"}';
            var discountAmountCol = '{"label": "Discount Amount", "value": "buildertek__Discount_Amount"}';
            var markupCol = '{"label": "Markup %", "value": "buildertek__Markup"}';
            var markUpAmountCol = '{"label": "Markup Amount", "value": "buildertek__Markup_Amount"}';
            var subTotalCol = '{"label": "Sub Total Amount", "value": "buildertek__SubTotal_Amount"}';
            var grossCost = '{"label": "Gross Cost", "value": "buildertek__Gross_Cost"}';
            var extendedCost = '{"label": "Extended Cost", "value": "buildertek__Extended_Cost"}';
            var salesTaxCol = '{"label": "Sales Tax", "value": "buildertek__SalesTax"}';
            var totalCostCol = '{"label": "Total Cost", "value": "buildertek__Total_Cost"}';
        ...
        }else if(groupByCategory == true && costAdjustmentColumnsPresent == true)
        {
            //Add New Rows
            costAdjustmentColumnsPresent = false;
        }else if(groupByPhase == true && costAdjustmentColumnsPresent == true && groupByPhaseFromToggle==true)
        {
            //Add New Rows
            costAdjustmentColumnsPresent = false;
            ...
            for (var j = 0; j < groupedRecords.length; j++) {
              ...
              //  if(grpData[j].buildertek__Tax__c != undefined )
               ...
               // }
                ...
                for (var k = 0; k < record.length; k++) 
                {
                    ...
                    }else if(helper.isPresentInSlab(serviceCategory))
                    {
                       ...
                       // alert(discountRateVal);
                        if(discountRateVal != null && discountRateVal != undefined && 
                            discountRateVal[bomLineId] != null && discountRateVal[bomLineId] != undefined)
                        {
                   ...
                   /* if(serviceCategory != 'Fab' && serviceCategory != 'Complete' && helper.isNotPresentInSlab(serviceCategory) && serviceCategory != 'Install' && serviceCategory != 'Edge' && serviceCategory != 'Work Comp' && serviceCategory != 'Ins' &&
                    ...
                    if(serviceCategory != 'Fab' && /*serviceCategory != 'Complete'*/
                       ...
                       helper.isNotPresentInSlab(serviceCategory) && serviceCategory != 'Install' && 
                   ...
                   /* if(serviceCategory != 'Slab - Quartz')
                    ...
                    var discountAmount = grossCost * (parseFloat(slabDiscount)/ 100);
                   grossLessCost = grossCost - Math.abs(discountAmount) ;
                  ...
                  /* Comment added by Harika, regarding CAES-63 ticket, Date: 09-08-2022*/ 
                    if (serviceCategory == 'Insurance' || serviceCategory == 'Work Comp'  || productCode == '69201198'
                       || /*serviceCategory == 'Complete'*/serviceCategory == 'Fab & Install' || serviceCategory == 'Misc' ) // Add: serviceCategory == 'Misc'
                    {
                       // salestax = 0;
                ...
                // console.log('$$$$bomIdVsExtendCost FINAL--',bomIdVsExtendCost,'--contains--',bomIdVsExtendCost.has(grpData[j].Id));
                // console.log('@@costAdjustmentColumnsPresent3-', costAdjustmentColumnsPresent);
                if (costAdjustmentColumnsPresent == false) {
                ...
                //    record[lastindex + 6] = JSON.parse('{"fieldType": "CURRENCY", "Key": "buildertek__SalesTax", "Value": "' + parseFloat(salestax).toFixed(2) + '"}');
                   record[lastindex + 7] = JSON.parse('{"fieldType": "CURRENCY", "Key": "buildertek__SalesTax", "Value": "' + parseFloat(salestax).toFixed(2) + '"}');
    ...
    {
        if(listOfProductCodes != null && listOfProductCodes != undefined && listOfProductCodes.length > 0 )
        {
        ...
        if(bomDetais!= null && bomDetais!= undefined && bomDetais.length > 0  )
        {
            ...
            action.setCallback(this, function (response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    if(bomDetais[0].buildertek__Fabrication_Install_Rate__c == null || bomDetais[0].buildertek__Fabrication_Install_Rate__c == undefined || bomDetais[0].buildertek__Fabrication_Install_Rate__c == '')
                    {
                        bomDetais[0].buildertek__Fabrication_Install_Rate__c = JSON.stringify(productCodes);
                    }
                    else
                    {
                        this.checkAllProductCodes(component, event, helper,'buildertek__Fabrication_Install_Rate__c',productCodes);
                    }

                    if(bomDetais[0].buildertek__Fab_Only_Rate__c == null || bomDetais[0].buildertek__Fab_Only_Rate__c == undefined || bomDetais[0].buildertek__Fab_Only_Rate__c == '')
                    {
                        bomDetais[0].buildertek__Fab_Only_Rate__c = JSON.stringify(productCodes);
                    }
                    else
                    {
                        this.checkAllProductCodes(component, event, helper,'buildertek__Fab_Only_Rate__c',productCodes);
                    }

                    if(bomDetais[0].buildertek__Install_Only__c == null || bomDetais[0].buildertek__Install_Only__c == undefined || bomDetais[0].buildertek__Install_Only__c == '')
                    {
                        bomDetais[0].buildertek__Install_Only__c = JSON.stringify(productCodes);
                    }
                    else
                    {
                        this.checkAllProductCodes(component, event, helper,'buildertek__Install_Only__c',productCodes);
                    }

                    if(bomDetais[0].buildertek__Mark_Up_Non_Slab_or_Labor__c == null || bomDetais[0].buildertek__Mark_Up_Non_Slab_or_Labor__c == undefined || bomDetais[0].buildertek__Mark_Up_Non_Slab_or_Labor__c == '')
                    {
                        bomDetais[0].buildertek__Mark_Up_Non_Slab_or_Labor__c = JSON.stringify(productCodes);
                    }
                    else
                    {
                        this.checkAllProductCodes(component, event, helper,'buildertek__Mark_Up_Non_Slab_or_Labor__c',productCodes);
                    }
    ...
    checkAllProductCodes: function (component, event, helper,fieldName,updatedProdCodes)
    {
        var bomDetais = component.get('v.bomList');
        ...
        for (var i = 0; i < updatedProdCodes.length; i++) {
            if (!allProdCodesFromDBArray.includes(updatedProdCodes[i].label)) {
                var jsonStr = '{"label": "' + updatedProdCodes[i].label + '", "value":""}';
        ...
        if(allProdCodes != null && allProdCodes != undefined && allProdCodes.length > 0 )
        {
            ...
            if(allRefinedproductCodes != null && allRefinedproductCodes != undefined && allRefinedproductCodes.length > 0 )
            {
                allProdCodes = [];
    ...
    calculateTotalCostAdjustment: function (component, event, helper) 
    {
        ...
        if(totalRecords!= null && totalRecords != undefined && totalRecords.length > 0)
        {
      ...
      //  alert('allData'+allData);
        for (var i = 0; i < allData.length; i++)
        {
            ...
            var grpData = allData[i].groupData;
            if(recordList != null && recordList != undefined && recordList.length > 0)
            {
                ...
                for (var j = 0; j < recordList.length; j++) 
                {
                    ...
                    if (grpData[j].buildertek__Takeoff_Line__r != null &&
                       ...
                       /* if(!serviceCatVsExtendedCost.has(grpData[j].buildertek__Service_Category__c))
                        ...
                        if(!serviceCatVsTotalCostMap.has(grpData[j].buildertek__Service_Category__c))
                        {
                            ...
                            if(!serviceCatVsExtendedCost.has(grpData[j].buildertek__BL_SERVICE_CATEGORY__c))
                        {
                            serviceCatVsExtendedCost.set(grpData[j].buildertek__BL_SERVICE_CATEGORY__c,'0');
                        }
                        if(!serviceCatVsTotalCostMap.has(grpData[j].buildertek__BL_SERVICE_CATEGORY__c))
                        {
                            serviceCatVsTotalCostMap.set(grpData[j].buildertek__BL_SERVICE_CATEGORY__c,'0');
                        }
                    }
                    if(grpData[j].buildertek__Product__r != null &&
                        grpData[j].buildertek__Product__r != undefined &&
                    ...
                    if(grpData[j].buildertek__OCIP_CCIP_Required__c != null &&
                    ...
                    for (var k = 0; k < record.length; k++) 
                    {
                        ...
                        if (record[k].Key == "buildertek__Total_Cost" && record[k].Value != null && record[k].Value != undefined &&
                        serviceCategory != null && serviceCategory != undefined && serviceCategory != '' 
                        ) 
                        {
                            /* Comment added by Harika, regarding CAES-54 ticket, Date: 26-07-2022*/
                        ...
                        // }
                        if (record[k].Key == "buildertek__Total_Cost" && record[k].Value != null && record[k].Value != undefined &&
                        /*(*/thisBuildPhaseName == 'Option'/* || serviceCategory == 'Option')*/ && thisProductCode != '69201198') {
                        ...
                        if (record[k].Key == "buildertek__Extended_Cost" && record[k].Value != null && record[k].Value != undefined && thisProductCode != '69201198') 
                        {
                            if(thisBuildPhaseName != null && thisBuildPhaseName != undefined && thisBuildPhaseName != '' && thisBuildPhaseName == 'Base')
                            {
                                extendedcost = extendedcost + parseFloat(record[k].Value);
                            }
                            if(thisBuildPhaseName != null && thisBuildPhaseName != undefined && thisBuildPhaseName != '' && serviceCategory == 'OCIP'){
                                ocipextendedcost = ocipextendedcost + parseFloat(record[k].Value);
                            }
                            
                            if ( thisBuildPhaseName == 'Base' && serviceCatVsExtendedCost.has(serviceCategory) ) 
                            {
                                let existingExtendedCost = Number(serviceCatVsExtendedCost.get(serviceCategory));
                                let currentVal = Number(record[k].Value);
                                let updatedVal = existingExtendedCost + currentVal;
                                updatedVal = parseFloat(updatedVal).toFixed(2);
                                serviceCatVsExtendedCost.set(serviceCategory,String(updatedVal));
                            }
                        }
                        if (record[k].Key == "buildertek__Gross_Cost" && record[k].Value != null && record[k].Value != undefined && thisProductCode != '69201198') 
                        {
                            if(thisBuildPhaseName != null && thisBuildPhaseName != undefined && thisBuildPhaseName != '' && thisBuildPhaseName == 'Base')
                            {
        ...
        if (markup != null && markup != undefined && markup != 0 &&
            totalBomLinesCount != null && totalBomLinesCount != undefined && totalBomLinesCount != 0) 
        {
    ...
    reCalculateTable: function (component, event, helper,installCost,proposalAmount,ocipextendedcost)
    {
        ...
        var ocip_ccip_req;
        let glRateValue = 0.00;
        let wcRateValue = 0.00;
        var bomList =[]; 
        bomList = component.get('v.bomList');
        if(bomList != null && bomList != undefined)
        {
                ...
                if (Textura != null && Textura != undefined && Textura.length > 0)
                {
                    ...
                    if (rate != null && rate != undefined && proposalAmount != null && proposalAmount != undefined) 
                    {
                        ...
                        if (texturaExtendedCost != null && texturaExtendedCost != undefined &&
                            ...
                            Textura[0].proposalAmountVal = parseFloat(proposalAmount).toFixed(2);
                ...
                if (generalLiability != null && generalLiability != undefined  && generalLiability.length > 0) 
                {
                    ...
                    if (proposalAmount != null && proposalAmount != undefined &&
                        ...
                        if (rateVal != null && rateVal != undefined) {
                            ...
                            glRateValue = rateVal;
                            generalLiability[0].proposalAmountVal = parseFloat(proposalAmount).toFixed(2);
                            bomlineIdVsGLFee.set(theBomLine, rateVal);
                        }

                        bomList[0].buildertek__General_Liability_Insurance_Long__c = JSON.stringify(generalLiability);
                    }
                }
            }
                ...
                if (workersComp != null && workersComp != undefined  && workersComp.length > 0) 
                {
                    ...
                    if (installCost != null && installCost != undefined &&
                        ...
                        if (rateVal != null && rateVal != undefined) {
                            ...
                            wcRateValue = rateVal;
                            workersComp[0].installCostVal = parseFloat(installCost).toFixed(2);
                            bomlineIdVsWCFee.set(theBomLine, rateVal);
                        }

                        bomList[0].buildertek__Workers_Comp__c = JSON.stringify(workersComp);
                    }
                }
            }
    ...
    getExtendedCosts: function (component, event, helper)
    {
        ...
        action.setCallback(this, function (response) 
        {
            ...
            if (state === "SUCCESS") 
            {
                ...
                var fabLaborMarkup = 'buildertek__Fabrication_Labor_Mark_up_LongText__c';
                var optionsRate = 'buildertek__Options_Rate__c';
                var workersComp = 'buildertek__Workers_Comp__c';
                var generalLiability = 'buildertek__General_Liability_Insurance_Long__c';
                var texturaFee = 'buildertek__Textura_Fee__c';

                if(extendedCostRecords[fabricationInstall] != null && extendedCostRecords[fabricationInstall] != undefined)
                {
                    component.set('v.fabInstallExtndCost',extendedCostRecords[fabricationInstall]);
                }
                if(extendedCostRecords[fabOnly] != null && extendedCostRecords[fabOnly] != undefined)
                {
                    component.set('v.fabOnlyExtndCost',extendedCostRecords[fabOnly]);
                }
                if(extendedCostRecords[installOnly] != null && extendedCostRecords[installOnly] != undefined)
                {
                    component.set('v.installOnlyExtndCost',extendedCostRecords[installOnly]);
                }
                if(extendedCostRecords[nonSlab] != null && extendedCostRecords[nonSlab] != undefined)
                {
                    component.set('v.nonSlabExtndCost',extendedCostRecords[nonSlab]);
                }
                if(extendedCostRecords[misc] != null && extendedCostRecords[misc] != undefined)
                {
                    component.set('v.miscExtndCost',extendedCostRecords[misc]);
                }
                if(extendedCostRecords[edge] != null && extendedCostRecords[edge] != undefined)
                {
                    component.set('v.edgeCost',extendedCostRecords[edge]);
                }
                if(extendedCostRecords[slabDiscount] != null && extendedCostRecords[slabDiscount] != undefined)
                {
                    component.set('v.slabDiscountRate',extendedCostRecords[slabDiscount]);
                }
                if(extendedCostRecords[fabLaborMarkup] != null && extendedCostRecords[fabLaborMarkup] != undefined)
                {
                    component.set('v.fabLaborMarkupRate',extendedCostRecords[fabLaborMarkup]);
                }
                ...
                if(extendedCostRecords[generalLiability] != null && extendedCostRecords[generalLiability] != undefined)
                {
                    var recordDetail = extendedCostRecords[generalLiability]
                    component.set('v.generalLiabilityCost',recordDetail[generalLiability]);
                }
                if(extendedCostRecords[texturaFee] != null && extendedCostRecords[texturaFee] != undefined)
                {
    ...
    setCostAdjustmentTableColumns: function (component, event, helper)
    {
        ...
        component.set( 'v.adjustmentTableColumns', [    
            ...
            //     iconPosition: 'left'
            // }},
            {label: 'Local Sales Tax', fieldName: 'buildertek__Tax_Rate_1__c', type: 'number', editable:'true',cellAttributes: { iconName: 'utility:percent',iconPosition: 'right' }},
            ...
            {
                label: 'Fabrication and Install Rate', fieldName: 'buildertek__Fabrication_Install_Rate__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Fabrication Only Rate', fieldName: 'buildertek__Fab_Only_Rate__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Install Only', fieldName: 'buildertek__Install_Only__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Edge Rate', fieldName: 'buildertek__Edge__c', type: 'button', typeAttributes: {
            ...
            {label: 'Tax on Fabrication', fieldName: 'buildertek__Tax_on_Fabrication_Required__c', 
             type: 'boolean', cellAttributes: { class: 'common-edit-col' },editable:'true'},
            {label: 'Labor Only Sales Tax', fieldName: 'buildertek__Tax_Rate_2__c', type: 'number', editable:'true',cellAttributes: { iconName: 'utility:percent',iconPosition: 'right' }, typeAttributes: { required: false }},  
            ...
            {
                label: 'Slab Discount Rate', fieldName: 'buildertek__Slab_Discount_Rate_LongText__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Fabrication / Labor Mark-up', fieldName: 'buildertek__Fabrication_Labor_Mark_up_LongText__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Mark Up (Non Slab) or (Non Labor)', fieldName: 'buildertek__Mark_Up_Non_Slab_or_Labor__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'OCIP/CCIP Required', fieldName: 'buildertek__OCIP_CCIP_Required__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Workers Comp', fieldName: 'buildertek__Workers_Comp__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'General Liability Insurance', fieldName: 'buildertek__General_Liability_Insurance_Long__c', type: 'button', typeAttributes: {
            ...
            {label: 'OCIP/CCIP', fieldName: 'buildertek__OCIP_CCIP__c', type: 'currency',typeAttributes: { currencyCode: 'USD', currencyDisplayAs : 'symbol',minimumFractionDigits :2,maximumFractionDigits: 2}},
            {
                label: 'Textura Fee', fieldName: 'buildertek__Textura_Fee__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Miscellaneous Rates', fieldName: 'buildertek__Miscellaneous_Rates__c', type: 'button', typeAttributes: {
            ...
            {
                label: 'Options', fieldName: 'buildertek__Options_Rate__c', type: 'button', typeAttributes: {
    ...
    validateData: function (component, event, helper)
    {
        ...
        if(updatedRecords != null && updatedRecords && updatedRecords.length > 0 && updatedRecords[0].buildertek__Tax_on_Fabrication_Required__c != null && updatedRecords[0].buildertek__Tax_on_Fabrication_Required__c == true )
        {
            //If Tax rate 2 is blank or 0 then show error
            //Check the DB value also
            if((updatedRecords[0].buildertek__Tax_Rate_2__c != null && 
                updatedRecords[0].buildertek__Tax_Rate_2__c != undefined && 
                (updatedRecords[0].buildertek__Tax_Rate_2__c == '' || parseInt(updatedRecords[0].buildertek__Tax_Rate_2__c) == 0 ))
                ||
                ((updatedRecords[0].buildertek__Tax_Rate_2__c == null || 
                    updatedRecords[0].buildertek__Tax_Rate_2__c == undefined) && (dataFromDB[0].buildertek__Tax_Rate_2__c == null || dataFromDB[0].buildertek__Tax_Rate_2__c == undefined || (dataFromDB[0].buildertek__Tax_Rate_2__c == '' || parseInt(dataFromDB[0].buildertek__Tax_Rate_2__c) == 0 )))
                )
            {
        ...
        if(updatedRecords != null && updatedRecords && updatedRecords.length > 0 && updatedRecords[0].buildertek__Tax_Rate_2__c != null && updatedRecords[0].buildertek__Tax_Rate_2__c != undefined && (parseInt(updatedRecords[0].buildertek__Tax_Rate_2__c) == 0 || updatedRecords[0].buildertek__Tax_Rate_2__c == '') )
        {
            if(updatedRecords[0].buildertek__Tax_on_Fabrication_Required__c != null && updatedRecords[0].buildertek__Tax_on_Fabrication_Required__c != undefined && updatedRecords[0].buildertek__Tax_on_Fabrication_Required__c == true )
            {
                helper.showToast(component, event, helper, 'Error!', 'Tax Rate 2 cannot be 0 when Tax on Fabrication is Required.', 'error');
                return false;
            }
            else if(dataFromDB[0].buildertek__Tax_on_Fabrication_Required__c != null && dataFromDB[0].buildertek__Tax_on_Fabrication_Required__c != undefined && dataFromDB[0].buildertek__Tax_on_Fabrication_Required__c == true)
            {
                helper.showToast(component, event, helper, 'Error!', 'Tax Rate 2 cannot be 0 when Tax on Fabrication is Required.', 'error');
                return false;
            }
        ...
        if(updatedRecords != null && updatedRecords && updatedRecords.length > 0 && updatedRecords[0].buildertek__OCIP_CCIP_Required__c != null && updatedRecords[0].buildertek__OCIP_CCIP_Required__c == true )
        {
            if((updatedRecords[0].buildertek__Workers_Comp_Insurance_Rate__c != null && 
                updatedRecords[0].buildertek__Workers_Comp_Insurance_Rate__c != undefined && 
                ...
                updatedRecords[0].buildertek__General_Liability_Insurance__c != undefined && 
        ...
        if(updatedRecords != null && updatedRecords && updatedRecords.length > 0 &&
            (updatedRecords[0].buildertek__Workers_Comp_Insurance_Rate__c != null && 
                updatedRecords[0].buildertek__Workers_Comp_Insurance_Rate__c != undefined && 
                updatedRecords[0].buildertek__Workers_Comp_Insurance_Rate__c == '') ||
                (updatedRecords[0].buildertek__General_Liability_Insurance__c != null && 
                ...
                updatedRecords[0].buildertek__General_Liability_Insurance__c == '') )
        {
            if( dataFromDB != null && dataFromDB != undefined && dataFromDB[0].buildertek__OCIP_CCIP_Required__c != null && dataFromDB[0].buildertek__OCIP_CCIP_Required__c != undefined && dataFromDB[0].buildertek__OCIP_CCIP_Required__c == true)
            {
                helper.showToast(component, event, helper, 'Error!', '"Workers Comp Insurance Rate" and "General Liability Insurance" must be entered when OCIP/CCIP is Required.', 'error');
                return false;
            }
